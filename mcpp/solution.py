...
from lsmcpp.rtc_planner import RTCPlanner
...
def MSTCStar_sol(mcpp:MCPP) -> Solution:
    ...
    class ModifiedMSTCStarPlanner(STCPlanner):
        ...
        def __sim(self, depot, serv_pts):
            ...
            path.extend([depot] + self.shortest_path_H(depot_small, serv_pts[0]))
            L, num_of_served = len(serv_pts), 1

            for i in range(L-1):
                if num_of_served == self.capacity:
                    num_of_served = 0
                    beta = self.shortest_path_H(path[-1], depot_small)
                    alpha = self.shortest_path_H(depot_small, serv_pts[i])
                ...
                if l1 != 0.5:
                    gamma = self.shortest_path_H(serv_pts[i], serv_pts[i+1])
            ...
            if path[-1] != depot:
                path.extend(self.shortest_path_H(path[-1], depot_small)[1:] + [depot])
        ...
        def shortest_path_H(self, start, goal):
            dist = lambda p, q: np.hypot(p[0]-q[0], p[1]-q[1])
            return nx.astar_path(self.H, start, goal, heuristic=dist, weight='weight')
